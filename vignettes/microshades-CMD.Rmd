...
cdf_britol <- color_objs_britol$color_df
```
...
cdf_HMP <- color_objs_HMP$color_df
```
...
britol_legend <- get_legend(plot_britol)
...
In this case, the function `match_color_df()` can apply the color factoring information from one processed mdf to a different, unprocessed mdf.
...
mdf_HMP_matched <- match_color_df(mdf_HMP_pre, mdf_britol)

# Use the Britol cdf for color reference
plot_HMP_matched <- plot_microshades(mdf_HMP_matched, cdf_britol)
